...
<div class="about" lang="${e}">
  ...
  `}function ue(){return'<svg viewBox="0 0 24 24"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>'}function K(){return'<svg viewBox="0 0 24 24"><path d="M12 3l2.6 6.3 6.9.6-5.2 4.5 1.6 6.7L12 17.8 5.1 21.1l1.6-6.7L1.5 9.9l6.9-.6z"/></svg>'}function de(){return'<svg viewBox="0 0 24 24"><path d="M12 3v2M12 19v2M3 12h2M19 12h2"/><circle cx="12" cy="12" r="4"/></svg>'}function Ee(e){var a;const t=Math.max(12,e);(a=document.getElementById("radar"))==null||a.style.setProperty("--r",`${t}px`)}function Le(e,t){const a=I(t,"Europe/Berlin"),n=document.getElementById("time-label"),s=document.getElementById("now-chip"),r=document.querySelector("#time-chip .dot");e.mode==="live"?(n.textContent=`Jetzt · ${S(a.hour,a.minute)}`,s.hidden=!0,r.style.background="#3BD37E"):(n.textContent=S(e.hour,0),s.hidden=!1,r.style.background="#E8C158")}function ke(e){const t=document.getElementById("banner");if(!e){t.classList.remove("show"),t.textContent="";return}t.textContent=e,t.classList.add("show")}function xe(e,t){const a=document.getElementById("cheap");document.getElementById("cheap-txt").textContent=e,a.classList.toggle("empty",t)}function Re(e){const t=document.getElementById("radius-val");t&&(t.textContent=`${e} km`);const a=document.getElementById("radius");a&&Number(a.value)!==e&&(a.value=String(e))}function Te(e){var t;(t=document.getElementById("empty"))==null||t.classList.toggle("show",e)}function Ce(e){const t=document.getElementById("sheet-root"),a=document.getElementById("sheet");a.innerHTML=e,t.classList.add("open")}function Be(){var e;(e=document.getElementById("sheet-root"))==null||e.classList.remove("open")}function Ie(e,t,a,n,s=0){var m,y;const r=t.map(p=>p.price).filter(p=>p!=null),i=Math.max(...r,.01),o=e.mode==="manual"?e.hour:a,c=t.map(p=>{const $=p.price==null?8:10+p.price/i*70,w=D.accent;return`<button type="button" class="${p.hour===o?"bar sel":"bar"}" data-hour="${p.hour}" style="height:${$}px;background:${w}" aria-label="${p.hour} Uhr"></button>`}).join(""),l=((m=t[o])==null?void 0:m.price)??null,h=l==null?"":`<span class="time-price">${k(l,n)}</span>`,d=((y=t[a])==null?void 0:y.price)??null,b=e.mode==="manual"?`<div class="time-now">Jetzt ${S(a,s)} · ${d==null?"—":L(d)}</div>`:"",f=le(t),g=f==null?"":`<div class="time-foot"><span class="dot"></span>Günstig: ${ce(f)} · ${k(f.price,n)}</div>`;return`
    ...
    <div class="sheet-body">
      ...
      ${g}
    </div>`}function ze(e,t,a,n,s,r,i,o){const c=F(e,t,a),l=c==null?null:B(c/C,e.currency,s,n),h=I(a,e.timezone),d=["So","Mo","Di","Mi","Do","Fr","Sa"],b=t.mode==="live"?`Jetzt · ${S(h.hour,h.minute)}`:`${d[h.weekdayJs]??""} ${S(t.hour,0)}`.trim(),f=l==null?'<div class="price-xl muted">Kein Preis</div>':`<div class="price-xl">${k(l,s)}</div>`,g=[],m=[];for(let v=0;v<24;v++){const x=P(e,v,a),E=x==null?null:B(x/C,e.currency,s,n);E!=null&&m.push(E),g.push({hour:v,val:E})}const y=Math.max(...m,.01),p=t.mode==="manual"?t.hour:h.hour,$=g.map(v=>{const x=v.val==null?6:10+v.val/y*70,E=D.accent;return`<button type="button" class="${v.hour===p?"bar sel":"bar"}" data-hour="${v.hour}" style="height:${x}px;background:${E}"></button>`}).join(""),w=m.length?Math.min(...m):null,R=m.length?Math.max(...m):null,H=w==null||R==null?"":`<span class="tages-minmax">min ${L(w)} · max ${L(R)}</span>`,U=`https://www.tesla.com/de_DE/findus/location/supercharger/${e.id}`,O=`geo:${e.lat},${e.lon}?q=${e.lat},${e.lon}(${encodeURIComponent(e.name)})`,W=e.stallCount!=null&&Number.isFinite(e.stallCount)?`<span class="meta">${e.stallCount} Stellplätze</span>`:"",j=e.maxPowerKw!=null&&Number.isFinite(e.maxPowerKw)?`<span class="meta">${e.maxPowerKw} kW</span>`:"";return`
    <div class="grab"></div>
    <div class="sheet-head">
      <div style="flex:1">
        <h2>${u(e.name||A(e))}${o?'<span class="tag">GÜNSTIG</span>':""}</h2>
    ...
    <div class="sheet-foot">
      <button class="primary" id="send-tesla" type="button" data-url="${u(U)}" data-name="${u(e.name)}" data-price="${l==null?"":u(k(l,s))}">An Tesla senden</button>
      <div class="linkrow">
    ...
    <div class="sheet-body">${e.length===0?'<p class="muted">Keine Favoriten.</p>':e.map(a=>{const n=a.station,s=[n.address.city,n.country].filter(Boolean).join(", "),r=a.price==null?"":k(a.price,a.displayCcy);return`<button class="fav-item" type="button" data-fav="${u(n.id)}"><span class="ampel-dot" style="background:${u(a.color)}"></span><span class="fav-meta"><span class="nm">${u(n.name||A(n))}</span><span class="muted">${u(s)}</span></span><span class="fav-price" style="color:${u(a.color)}">${u(r)}</span><span class="chev">›</span></button>`}).join("")}</div>`}function u(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}export{ze as A,D as C,me as F,C as M,Re as a,ve as b,B as c,Ee as d,xe as e,L as f,Le as g,I as h,Ue as i,$e as j,De as k,ye as l,Se as m,Te as n,Ce as o,ke as p,we as q,Be as r,F as s,Ie as t,Ne as u,ge as v,be as w,fe as x,k as y,A as z};
